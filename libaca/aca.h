...
/** Write a new gateway property to the server.
 *
 ...
 */
arrayent_return_e ArrayentRecvProperty(uint8_t* user_index, char* data, uint16_t* len, uint32_t timeout);


/** Login for child devices.
 *
 * This function returns a bit field indicating the network status of the ACA.
 *
 * @param[in]  index - current available child device index is 1~15.
 *
 * @param[in] code - a unique code assigned by Arrayent.
 *
 * @param[in] password - a unique device password assigned by Arrayent.
 *
 * @return    Indicates if the status was updated.
 */
arrayent_return_e ArrayentChildLogin(uint8_t index, char* code, char* passoword);
 ...
 * @return    Indicates if the status was updated.
 */
arrayent_return_e ArrayentChildLogout(uint8_t index);

/*
  Write a new child device property to the server.
 *
 * This function sets an arbitrary property on the Arrayent servers.
 *
 * @para[in] index - the index used in function ArrayentChildLogin.
 *
 * @param[in] property - the property send to the server.
 *
 * @return     Indicates if the property was set correctly.
 *
 */
arrayent_return_e ArrayentSetChildProperty(uint8_t index, char* property);
